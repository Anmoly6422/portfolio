        ...
        <div className="flex flex-col -mt-8 text-4xl md:text-5xl lg:text-6xl xl:text-7xl">
            {["Home","Services","About","Work","Contact"].map((section,index)=>(
                <div key={index} ref={(el)=>(linkRef.current[index]= el)}>
                    <Link className='transtion-all duration-300 cursor-pointer hover:text-white'
                    to={`${section}`}
                    smooth
                    offset={0}
                    duration={2000}>
        ...
        <div ref={contactRef}
         className='flex flex-col flex-wrap justify-between gap-8 md:flex-row'>
            <div className='font-light'>
                ...
                <p className='text-xl tracking-widest lowercase text-pretty'>yadavanmol540@gmail.com</p>
            </div>
            <div className='font-light'>
                ...
                <div className='flex flex-col flex-wrap md:flex-row gap-x-2'>
                    ...
                    href={social.href}
                    key={index} className='text-sm leading-loose
                    tracking-widest uppercase hover:text-white transition-colors duration-300'>{social.name}</a>))}
                </div>
            </div>
